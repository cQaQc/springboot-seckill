...
**1.项目初始化:** 项目启动，商品信息缓存进redis

**2.登录模块:** 查主键是否存在---判断用户是否存在，先从redis缓存查，缓存里查不到再查数据库，用户查到了缓存进到redis，验证密码，验证成功生成Token放进Cookie，返回结果、token

**3.商品模块:**

展示商品列表：先从缓存拿列表页面，没有再从数据库查询商品列表，渲染商品列表页面存进缓存，反给前端。

商品详情： 通过id从数据库查，展示在页面

秒杀： RateLimiter使用令牌桶算法进行限流，acquire不到令牌返回友好提示，acquire到令牌预减库存，缓存库存小于零查库更新缓存，判断用户重复秒杀，下订单减库存在同一个事务
